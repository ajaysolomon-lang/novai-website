...
import { handleVoiceWebhook, handleCallLog, handleAnalytics, handleHealth, handleTestSeed, handleClearData, handleVapiData } from './voice-agent.js';
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/_wb-voice/")) {
      ...
      if (url.pathname === "/_wb-voice/test" && request.method === "DELETE") {
        return handleClearData(request, env);
      }
      if (url.pathname === "/_wb-voice/vapi-data" && request.method === "GET") {
        return handleVapiData(request, env);
      }
